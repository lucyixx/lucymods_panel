<div class="navbar bg-base-200 border-b border-base-300 px-3 max-w-5xl mx-auto w-full">
    <div class="flex-1">
    ...
    <div class="hidden md:flex flex-none items-center gap-1">
        <!-- Theme toggle: daisyUI's own theme-controller + swap, no custom JS needed -->
...
<div class="hidden md:hidden flex-col border-b border-base-300 bg-base-200 px-3 pb-3 max-w-5xl mx-auto w-full" id="navbarSupportedContent">
    ...
    <?php endif; ?>
    <label class="swap swap-rotate btn btn-ghost btn-sm justify-start mt-1">
        <input type="checkbox" class="theme-controller" value="zygame-light" />
        <span class="swap-on"><i class="bi bi-sun"></i> Light mode</span>
        <span class="swap-off"><i class="bi bi-moon-stars"></i> Dark mode</span>
    </label>
</div>
...
<script>
    ...
    // Persist the daisyUI theme choice (theme-controller handles the actual
    // switch; this just remembers it for the next page load, since pages
    // are separate full loads in this multi-page app, not an SPA).
